...
@injectable()
export class ParseUsecase {
  /**
   * 配信サイト名の判定に使用する特徴的なキーワード
   */
  private STREAMING_SERVICE_KEYWORDS = [
    'Netflix',
    'Amazon',
    'dアニメストア',
    'Hulu',
   ...
   */
  async parseAnnictPage(response: Response): Promise<AnnictPageInfo> {
    // 結果を格納するオブジェクト
    ...
    $('ul').each((index, ul) => {
      ...
      $ul.find('a').each((_, a) => {
        ...
        if (this.isStreamingService(text)) {
    ...
    if (foundStreamingUl && streamingUlIndex >= 0) {
      ...
      streamingUl.find('a').each((_, a) => {
        ...
        if (this.isStreamingService(text) && href) {
   ...
   */
  private isStreamingService(text: string | null): boolean {
    if (!text) {
      return false;
    }

    return this.STREAMING_SERVICE_KEYWORDS.some((keyword) =>
